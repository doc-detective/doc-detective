  ...
  for (const item of items) {
    ...
    if (ref.kind === "browser") {
      ...
      for (const handle of targets.handles) {
         ...
         * window's children then its lead), and the survivors>0 guard on lines
         * 108-113 has already refused this entry unless a non-closing user tab
         * survives. So in this loop closeHandle never trips its own last-tab
         * guard (a survivor always remains), never resolves entry-not-found
         * (each distinct target is still open on its turn), and always finds a
         * `next` handle to focus -- it cannot return ok:false here. No hermetic
         * input drives this branch (ADR 01017). */
        if (!closedResult.ok) {
          result.status = "FAIL";
