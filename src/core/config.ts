...
async function setConfig({ config }: any) {
  ...
  // Replace fileType strings with objects. Deep-copy the default so later
  // in-place normalization (and any caller mutation) can't contaminate the
  // shared table that common and the LSP also read.
  config.fileTypes = config.fileTypes.map((fileType: any) => {
    ...
    if (typeof fileTypeObject !== "undefined")
      return JSON.parse(JSON.stringify(fileTypeObject));
    log(
      config,
      "error",
