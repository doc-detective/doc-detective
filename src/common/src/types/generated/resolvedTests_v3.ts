...
export interface Config {
  ...
  output?: string;
  /**
   * Reporters to use when emitting test results. Built-in reporters: `terminal`, `json`, `html`, `runFolder`, `junit` (JUnit XML at `<output>/junit.xml` for CI test-summary widgets), `markdown` (a run summary at `<output>/doc-detective-summary.md` for CI job summaries and merge request comments). The `runFolder` reporter (on by default) archives each run's results as `<output>/.doc-detective/runs/<runId>/testResults.json` (or `coverageResults.json` for coverage runs), beside any screenshots the run captured, in addition to the flat output the `json` reporter writes. You can also reference custom reporters registered via `registerReporter()` by name.
   */
  reporters?: string[];
  /**
...
export interface AnnotationDefaults1 {
   ...
   */
  intensity?: number;
  outline?: AnnotationStyle6;
  arrow?: AnnotationStyle7;
  badge?: AnnotationStyle8;
  callout?: AnnotationStyle9;
  blur?: AnnotationStyle10;
  text?: AnnotationStyle11;
