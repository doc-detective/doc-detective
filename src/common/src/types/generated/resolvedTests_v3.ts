...
export type FileTypeCustom = {
   ...
   */
  extends?: "markdown" | "asciidoc" | "html";
  /**
   * File extensions to use with type.
   */
  extensions?: string | [string, ...string[]];
  inlineStatements?: InlineStatementDefinition;
  /**
   * Markup definitions for the file type.
   *
   * @minItems 1
   */
  markup?: [MarkupDefinition, ...MarkupDefinition[]];
  [k: string]: unknown;
} & (
  | {
      [k: string]: unknown;
 ...
 * Load environment variables from the specified `.env` file.
 */
export type LoadVariables2 = string;
/**
 * A condition expression, or an array of expressions combined with logical AND.
 */
 ...
 * Perform a native shell command.
 */
export type RunShell2 = RunShellCommandSimple1 | RunShellCommandDetailed1;
/**
 * Command to perform in the default shell (`bash` on every platform, unless the config-level `shell` setting changes it).
 */
...
export interface Config {
  ...
  /**
   * How many times to re-run a whole test context on a fresh session when the session it was running against turns out to be unusable — an early step (like `goTo`) passes, then a later step fails because the session or page is gone. After any failing context, an active probe checks the session and retries only when it finds one of three unambiguous signs: the session is dead (WebDriver `ECONNREFUSED` / `invalid session id`), it is alive but sitting on a browser crash or error page (`chrome-error:`, `about:neterror`), or it is alive but never left its initial blank document, meaning the context never actually got started. A page a test could legitimately be on is never treated as a retry signal, so a real assertion failure (the session is alive, the page loaded, the element genuinely isn't there) always fails and is never retried. Defaults to `1`; set to `0` to disable and keep the exact single-attempt behavior. Contexts can override this with their own `retries` field. This is distinct from a step's `retry` route, which re-runs a single step on the *same* session — `retries` re-runs the *whole context* on a *new* session. Capped at `10` to prevent a runaway retry storm from a misconfiguration.
   */
  retries?: number;
}
/**
 ...
 */
export interface InlineStatementDefinition {
  /**
   * Regular expressions that indicate the start of a test. If capture groups are used, the first capture group is used for the statement. If no capture groups are used, the entire match is used for the statement.
   */
  testStart?: string | [string, ...string[]];
  /**
   * Regular expressions that indicate that the current test is complete.
   */
  testEnd?: string | [string, ...string[]];
  /**
   * Regular expressions that indicates that the following content should be ignored for testing purposes.
   */
  ignoreStart?: string | [string, ...string[]];
  /**
   * Regular expressions that indicate that the ignored section of content is complete.
   */
  ignoreEnd?: string | [string, ...string[]];
  /**
...
export interface MarkupDefinition {
  /**
   * Name of the markup definition
   */
  name?: string;
  /**
   * Regular expressions to match the markup type.
   */
  regex?: string | [string, ...string[]];
  /**
   * If `true`, all matches are combined into a single string.
   */
  batchMatches?: boolean;
  /**
   * Actions to perform when the markup type is detected.
   */
  actions?:
    | (
        | "annotate"
...
export interface LoadVariables1 {
  loadVariables: LoadVariables2;
  [k: string]: unknown;
}
...
export interface IntegrationsOptions {
  openApi?: (OpenApi1 & OpenAPIDescriptionTest)[];
...
export interface AnnotationDefaults {
   ...
   */
  intensity?: number;
  outline?: AnnotationStyle3;
  arrow?: AnnotationStyle4;
  badge?: AnnotationStyle5;
  callout?: AnnotationStyle6;
  blur?: AnnotationStyle7;
  text?: AnnotationStyle8;
...
/**
 * Default visual theme for annotations in this spec's tests. Overrides the config-level `annotationDefaults`; individual tests can override this value with their own `annotationDefaults`. When unset, defers to the config level.
 */
export interface AnnotationDefaults1 {
  /**
   * Default foreground color for every annotation type.
   */
  color?: string;
   ...
   */
  intensity?: number;
  outline?: AnnotationStyle9;
  arrow?: AnnotationStyle10;
  badge?: AnnotationStyle11;
  callout?: AnnotationStyle12;
  blur?: AnnotationStyle13;
  text?: AnnotationStyle14;
 ...
 * Style overrides applied to every `callout` annotation.
 */
export interface AnnotationStyle12 {
  /**
   * Foreground color — strokes, arrowheads, and text (hex, rgb, or named color).
   */
  color?: string;
 ...
 * Style overrides applied to every `blur` annotation.
 */
export interface AnnotationStyle13 {
  /**
   * Foreground color — strokes, arrowheads, and text (hex, rgb, or named color).
   */
  color?: string;
 ...
 * Style overrides applied to every `text` annotation.
 */
export interface AnnotationStyle14 {
  /**
   * Foreground color — strokes, arrowheads, and text (hex, rgb, or named color).
   */
  color?: string;
