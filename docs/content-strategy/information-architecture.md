...
| Selectors / Visual matching (`image` criterion) | W2, W3 | ★ | `docs/selectors/visual-matching.mdx`; engine + thresholds must agree with `visualMatch.ts` and `elementImage_v3` |
...
- **Never hand-edit a `reference/schemas/*.mdx` page.** To change its content, change the JSON schema in
...
|---|---|---|
| Configuration (`config.mdx`) | yes (`buildSchemaReferencesV4.js`) | [`config_v3.schema.json`](../../src/common/src/schemas/src_schemas/config_v3.schema.json) |
| Contexts & surfaces (`context.mdx`) | yes | `context_v3.schema.json` |
| Actions (per-action `*.mdx`) | yes | per-action schemas in [`src/common/src/schemas/src_schemas/`](../../src/common/src/schemas/src_schemas/) |
| All other `reference/schemas/*.mdx` | yes | the corresponding schema |
