...
Her docs mostly walk readers through a **graphical interface**. She therefore lives in the
**UI procedures** sub-section of the "Test your docs" track. Diego uses that same track for CLI,
code, and API docs.
...
- Metadata and steps drift as the team and the product grow, but she has no enforcement mechanism.
  She has only review fatigue.

**How she uses Doc Detective:** writes inline or detected tests in her source files, runs them
locally, and wires a CI gate. She lets the tool auto-capture fresh screenshots and video. She returns
when adding coverage, supporting a new platform or browser, or tightening the suite.

**Why she is the lead persona:** she is the primary adopter and touches every layer. That covers
authoring tests, contexts, configuration, recording, and CI. Her anchor journey (W1) threads install →
authoring → contexts → CI in a single coherent path.

---

## Diego, Developer and SDK/API Engineer

Diego maintains code samples and API reference content. He has high technical proficiency and scripts
freely. He does the same job as Wren, keeping the docs correct. The procedures in his docs drive a
**terminal or an HTTP endpoint** rather than a UI. He therefore lives in the **CLI, code, and APIs**
...
**How he uses Doc Detective:** authors standalone `*.spec.json` files. He uses `runShell`, `runCode`,
and `runBrowserScript` to execute commands and scripts, and `httpRequest` and `checkLink` to exercise
APIs. He captures `outputs`, asserts on them, and generates API tests from an OpenAPI spec.

---

## Priya, Platform and CI Engineer
...
**How she uses Doc Detective:** runs the GitHub Action or `doc-detective-runner` in Docker. She sets
`--reporters json/html`, tunes `concurrentRunners`, and warms the cache with `install`. She can also
point at the orchestration API for distributed runs. She returns when adding a CI platform, or when
...
**How she uses Doc Detective:** runs `install agents` and works through the Claude Code, Copilot CLI,
or Gemini CLI integrations. She connects agent tools and MCP, and relies on self-healing to repair
tests. She follows best-practice prompts for agent-authored tests.
...
**How he uses Doc Detective:** follows the local-development guide and picks the standard or
substantial contribution flow. He consults the per-repo guides and content templates, then works
