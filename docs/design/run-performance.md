...
resolution, runner, driver and session, and reporting paths. Every finding below was verified
against code with file:line evidence. Refuted candidates are excluded: JIT-install network cost on
steady-state runs, per-call AJV schema compilation, and `recordRuntimeDependencies` on the
no-install path.
...
- **Per-file tax.** Spec files are read twice and validated 3–5× each, with a full
  ...
  Detection regexes are recompiled about 36 times per markdown file
...
- **Per-screenshot tax.** Captures are written to disk, read back and sync-decoded for compare,
  then re-encoded on size mismatch. The crop path does write → re-read → extract → rename
  ([saveScreenshot.ts:519](../../src/core/tests/saveScreenshot.ts), `:592-614`, `:654-655`,
  `:699-714`). That's up to ~5 PNG encode and decode passes, plus 4 disk ops per step.
- **Run-end tax.** The whole results tree is walked at least 4 times. That's the info-level dump at
  [core/index.ts:254-256](../../src/core/index.ts), the json reporter, the runFolder JSON, and the
  HTML build. The PostHog flush ([telem.ts:97-102](../../src/core/telem.ts)) then holds the event
  loop open after everything else finishes.

## Decisions (settled, 2026-07-14)

1. **Driver presence is a filesystem question, not an Appium question.** The `appium driver list`
   spawn is replaced by direct `resolveHeavyDepPath()` and `npmInstalled()` checks. That's the
   mechanism already used at [config.ts:1054-1092](../../src/core/config.ts). This serves *all*
   platforms identically, covering browser drivers and native Windows, macOS, iOS, and Android
   drivers. There's no gating on "browser runs only."
2. **Session reuse is default-on where reset is provably complete, and only there.** No opt-in
   ...
   without a verifiable global state clear, meaning Firefox and Safari, keep today's fresh-session
   behavior, as do native app surfaces. Any reset failure falls back to a fresh session (fail-closed). Users
...
3. **Self-update overlaps resolution; it still completes before the first test executes.**
   ...
   it from resolved config ([cli.ts:169-173](../../src/cli.ts)), so there's no new plumbing.
...
| 1.5 | `replaceEnvs`: hoist the constant regex to module scope. Substitute once before the retry loop instead of per attempt ([utils.ts:1461-1510](../../src/utils.ts), call at [tests.ts:5078](../../src/core/tests.ts)) | per step × attempt | allocations and redundant re-walks |
| 1.6 | Telemetry: call `sendTelemetry` before reporters and hints run, then `await client.shutdown(timeoutMs)` after they finish ([telem.ts:97-102](../../src/core/telem.ts), [core/index.ts:262](../../src/core/index.ts)). The flush overlaps reporter I/O, with a bounded worst case and no dropped events, since there's no `unref` | once per run | full PostHog round-trip off the exit path |
| 1.7 | Detection micro-fixes. Use a single `statSync`, or `readdirSync(..., { withFileTypes: true })` ([detectTests.ts:345-346](../../src/core/detectTests.ts), `:390-391`). Hoist `allowedExtensions` to a per-run `Set` (`:79-82`). Cache compiled `safeRegExp` per `(pattern, flags)`, and drop the no-op `Array.from` rebuild ([common detectTests.ts:20-31](../../src/common/src/detectTests.ts)) | per walked entry, per file | thousands of syscalls, and ~36 compiles per file |

1.7's regex cache lives in `src/common`, so follow [src/common/AGENTS.md](../../src/common/AGENTS.md)
and its tdd-coverage skill.

## Phase 2: startup path (contract-adjacent, needs ADRs)
...
ADRs are needed for three items. 2.1 makes the probe presence-based, so document the
"present ≠ functional" trade and the retained `verifyDriverBinary` layer. 2.3 covers the
update-check ordering guarantee, and 2.4 the terminal output contract change. On docs impact,
2.4 touches what users see at default log level.

## Phase 3: resolution and validation efficiency
...
| 3.2 | `validate()` clone strategy. Probe candidate schemas with non-mutating check-only validators, without `useDefaults`. Clone **once** for the winning mutate-with-defaults pass only. Today there's one clone per candidate, up to 12 for `step_v3` at [validate.ts:167](../../src/common/src/validate.ts). Use `structuredClone` on the detection path, where inputs are guaranteed JSON values from `JSON.parse` or YAML | [validate.ts:143](../../src/common/src/validate.ts), `:167` | dominant detection CPU on step-heavy specs |
...
3.2 changes `src/common` validation internals. The behavior contract is preserved and pinned by
tests before refactoring: callers' objects are never mutated, and the returned object carries
defaults. Write an ADR for 3.2's clone-semantics decision. 3.1 and 3.3–3.5 are mechanical and need
no ADR. 3.3 does need a fixture asserting OpenAPI-bound runs still resolve identically when
multiple tests share a description.
...
2. Crop on the buffer with `sharp(buffer).extract(...)`. That eliminates the write → `sharp(filePath)`
   re-read → `cropped_*.png` temp write → `renameSync` cycle (`:592-614`).
3. Compare decoded buffers directly. That eliminates `PNG.sync.read(fs.readFileSync(...))` read-backs
...
Same files land on disk with the same names and compare semantics. It's behavior-preserving, so no
ADR is needed. The existing capture fixtures in
[test/core-artifacts/capture/](../../test/core-artifacts/capture) gate it. Add a permutation
covering crop and variation-compare together, the worst-case path.
...
3. CDP global clears. This is why Chromium qualifies, since one call covers all storage classes:
   - `Storage.clearDataForOrigin(origin: "*", storageTypes: "all")` for cookies, local and session
     storage, IndexedDB, WebSQL, service workers, and cache storage.
   - `Network.clearBrowserCookies` and `Network.clearBrowserCache`, as belt-and-suspenders for the
     network stack.
   - `Browser.resetPermissions` for granted permission grants.
   - `Emulation.clearDeviceMetricsOverride` and `Emulation.clearGeolocationOverride` for any
...
- Reuse pool keyed by the full capability signature. That's engine, headless, args, and proxy,
  meaning everything except resettable state like window size. It's scoped per Appium server and
  runner worker, so the existing ChromeDriver-contention design
  ([tests.ts:1720](../../src/core/tests.ts)) is untouched.
...
reuse boundary for every state class**, per engine. The state classes are cookie, localStorage,
sessionStorage, IndexedDB, cache storage, service worker registration, granted permission,
viewport override, and window count. Each fixture is a pair of tests in one spec. Test A plants the state, and test B (same
...
Every phase lands with the repo's standard trio where applicable: an ADR (noted per phase),
fixtures, and a docs-impact assessment. For measurement, reuse the `report.warm`-style timing block
to add coarse phase timings for startup, resolution, execution, and reporting. Each phase's win is
then visible in run output rather than asserted.
