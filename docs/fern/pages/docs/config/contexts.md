...
The desktop platforms (`windows`, `mac`, and `linux`) run browser tests and, on Windows and macOS, [native desktop app tests](/docs/actions/startsurface). The mobile platforms run either browser (web) tests or native app tests on a managed device—one context drives one surface, not both. The context's [`device`](/reference/schemas/device-descriptor) field sets the default device: `android` runs on a managed emulator, and `ios` runs on a managed simulator. See [Run on an Android emulator](/docs/actions/startsurface#run-on-an-android-emulator) and [Run on an iOS simulator](/docs/actions/startsurface#run-on-an-ios-simulator) for native apps, and [Mobile browsers](#mobile-browsers-android-and-ios) for web tests. Because iOS tests run only on a macOS host, an `ios` context on Windows or Linux skips with a reason.
