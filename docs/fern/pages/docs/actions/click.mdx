...
- `surface`: (Optional) The browser window or tab the element lives in, for example `{ "browser": "chrome", "tab": "cart" }`, or the [native app surface](/docs/actions/startsurface) it lives in, for example `{ "app": "notepad" }`. Omit to act on the active browser tab. The targeted browser tab stays focused afterward. On an app surface, `elementText`, `elementId`, and `elementAria` map to native controls, and `selector` accepts a native locator (XPath or a `~`-prefixed accessibility ID) instead of a CSS selector. See [Test across multiple tabs and windows](/docs/test-docs/multiple-tabs-and-windows) and [Target an app surface](/docs/actions/startsurface#target-an-app-surface).
