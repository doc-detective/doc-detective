...
- `surface`: (Optional) The browser window or tab the element lives in, for example `{ "browser": "chrome", "tab": "cart" }`. Omit to act on the active tab. The targeted tab stays focused afterward. See [Test across multiple tabs and windows](/docs/test-docs/multiple-tabs-and-windows).
