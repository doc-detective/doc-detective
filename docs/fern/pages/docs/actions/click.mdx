...
**Outputs:** when you target an element by text or selector, the action exposes `outputs.found`, a boolean indicating whether the element was located. Reference it with the `$$outputs.*` namespace as `$$outputs.found`.
