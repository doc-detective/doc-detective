...
- `surface`: (Optional) The browser window or tab the element lives in, e.g. `{ "browser": "chrome", "tab": "cart" }`. Omit to act on the active tab; the targeted tab stays focused afterward. See [Test across multiple tabs and windows](/docs/test-docs/multiple-tabs-and-windows).
