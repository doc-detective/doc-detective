...
**Outputs:** the action exposes `outputs.sourceFound` and `outputs.targetFound`, booleans indicating whether the source and target elements were located. Reference them with the `$$outputs.*` namespace as `$$outputs.sourceFound` and `$$outputs.targetFound`.
