...
The `dragAndDrop` action requires both a source and a target element. Specify each with element text, a CSS or XPath selector, or a detailed object carrying more configuration options.

To move between **points** instead of elements, use [`swipe`](swipe), the movement subset of this action. That covers carousels, sliders, and plain scrolling. `dragAndDrop` locates two elements and moves between them, while `swipe` moves between two coordinates or a direction shorthand.
