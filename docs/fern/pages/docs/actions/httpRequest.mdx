...
Depending on which validations you configure, the action also exposes the result of each check in its `outputs` object, which you reference with the `$$outputs.*` namespace. Each output is present only when its check applies:

- `$$outputs.requiredFieldsPresent`: whether every field in `response.required` was found (when `response.required` is set).
- `$$outputs.bodyMatches`: whether the response body matched `response.body` (when `response.body` is set).
- `$$outputs.headersMatch`: whether the expected `response.headers` were present (when `response.headers` is set).
- `$$outputs.noUnexpectedFields`: whether the response contained only expected fields (when `allowAdditionalFields` is `false`).
- `$$outputs.responseSchemaValid`: whether the response matched the OpenAPI schema (when validating the response against an OpenAPI definition).
- `$$outputs.variation`: the fractional difference between the new response and the reference file (when you save the response with `path` and a reference file already exists).
