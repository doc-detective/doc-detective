...
**Setting Variables:** when you target a specific element with a selector or element text, the action exposes `$$found`, a boolean indicating whether the element was located. Reference it in later steps through the step-level `variables` object. When no element criteria are set, `type` types into the active element and no `$$found` output is produced.
