...
**Outputs:** when you target a specific element with a selector or element text, the action exposes `outputs.found`, a boolean indicating whether the element was located, which you reference with the `$$outputs.*` namespace as `$$outputs.found`. When no element criteria are set, `type` types into the active element and produces no `found` output.
