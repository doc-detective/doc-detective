...
For native apps, `{ "closeSurface": { "app": "notepad" } }` ends the app surface, terminating the app if Doc Detective launched it. `{ "closeSurface": { "app": "notepad", "window": -1 } }` closes just one window, a dialog say, and keeps the app open. Window selectors use the shared grammar of title, `/regex/`, and `-1` for the newest window. See [App windows](find#app-windows). Doc Detective refuses to close an app's **last** window and points at the bare form instead. A selector that matches nothing is a no-op.
...
One guardrail applies. Doc Detective refuses to close the **last open tab** of a browser. Closing a tab isn't how you end a session; closing the **whole browser** is.
