  ...
  "tests": [
    {
      "steps": [
        ...
        {
          "description": "Close the checkout tab by its URL.",
          "closeSurface": { "browser": "chrome", "tab": { "url": "/checkout" } }
        }
      ]
    }
  ]
...
To close a single window, such as a dialog, without ending the app, add a `window` selector. It accepts a window name, an index in creation order (`-1` is the newest), or `{ title }` criteria matched as a substring or `/regex/`:
  ...
  "tests": [
    {
      "steps": [
        {
          "description": "Close the newest window, keeping the app open.",
          "closeSurface": { "app": "notepad", "window": -1 }
        },
        {
          "description": "Close the Find dialog by its title.",
          "closeSurface": { "app": "notepad", "window": { "title": "Find" } }
        }
      ]
    }
  ]
