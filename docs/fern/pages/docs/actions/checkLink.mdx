...
**Outputs:** the action exposes `outputs.statusCode`, the HTTP status code returned by the request. Reference it with the `$$outputs.*` namespace as `$$outputs.statusCode`.
