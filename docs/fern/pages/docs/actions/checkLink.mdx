...
Include non-2xx status codes in `statusCodes` to accept responses that would otherwise fail, such as rate limiting (`429`) or authentication errors (`403`). As soon as a response matches `statusCodes`, Doc Detective stops and passes without retrying or falling back to `HEAD` (see [Behavior](#behavior)).
...
Append a testing token to the request URL with step-level `params`.
...
### Check a link from a variable

The string form and the object `url` both accept a `$VAR` reference, so you can check a link whose address comes from an environment variable loaded with [`loadVariables`](/docs/actions/loadvariables).
  ...
  "tests": [
    {
      "steps": [
        { "loadVariables": ".env" },
        {
          "description": "Check the link named by the DOCS_URL variable.",
          "checkLink": "$DOCS_URL"
        }
      ]
    }
...
### Combine every option

A single step can set `url`, `origin`, `params`, `statusCodes`, and `headers` together. It resolves a path against an origin, appends query parameters, sends a custom header, and accepts a chosen set of status codes.

{/* test {"testId":"checklink-combined","detectSteps":false} */}
  ...
  "tests": [
    {
      "steps": [
        {
          "description": "Check a protected API path with every option set.",
          "checkLink": {
            "url": "/api/health",
            "origin": "https://my-app.com",
            "params": {
...
{/* step {"stepId":"checklink-combined","description":"Check a path with origin, params, status codes, and a custom header all set","checkLink":{"url":"/status/200","origin":"http://localhost:8092","params":{"token":"test-value"},"statusCodes":[200,201],"headers":{"X-Doc-Detective-Check":"shared-secret"}}} */}
{/* test end */}
