...
The `checkLink` action checks if a URL returns an acceptable status code. It primarily issues a browser-mimicking `GET` request. If the response is still `429` or `403` after retries, it retries the URL once with a `HEAD` request as a fallback. This action is useful for verifying that a hyperlink or image URL is valid.
...
Doc Detective's retry-with-backoff and HEAD-fallback logic, described above in [Behavior](#behavior), only runs while the response code isn't yet accepted. As soon as a response matches `statusCodes`, Doc Detective stops immediately and passes. That includes a match on the very first `GET`, as in the example below. It doesn't retry or fall back to `HEAD` merely because the code happens to be `429` or `403`.
...
Sites behind bot protection or a web application firewall (WAF) can return `403` or `429` to non-browser traffic even when the URL works in your browser. Examples include Cloudflare, Akamai, Amazon Web Services (AWS) WAF, Imperva, PerimeterX, and Vercel deployment protection. Doc Detective already sends a browser-like header set and retries transient failures. Some WAFs still fingerprint deeper signals that header spoofing can't influence, such as Transport Layer Security (TLS) client fingerprints or source IP reputation.
...
If you own the site, add a WAF rule that allowlists a custom header and shared secret, such as `X-Doc-Detective-Check`. Prefer that over trying to mimic a browser.

### `checkLink` fails due to unrecognized certificates

The `checkLink` action might fail for a valid URL that loads without redirects. An internal or custom certificate the testing machine doesn't recognize is a common cause.

#### Example

Consider the following test configuration. It checks the validity of `https://self-signed.badssl.com/`, a website using a self-signed certificate:
