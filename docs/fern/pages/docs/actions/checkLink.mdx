...
Include non-2xx status codes in `statusCodes` to accept responses that would otherwise fail, such as rate limiting (`429`) or authentication errors (`403`). Once a response matches `statusCodes`, Doc Detective stops and passes without retrying or falling back to `HEAD` (see [Behavior](#behavior)).
