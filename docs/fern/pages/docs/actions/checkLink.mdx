...
**Setting Variables:** to capture the result for later steps, use the step-level `variables` object. The action exposes `$$statusCode`, the HTTP status code returned by the request.
