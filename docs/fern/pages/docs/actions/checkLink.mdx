...
Sites behind bot protection or a web application firewall (WAF), for example Cloudflare, Akamai, Amazon Web Services (AWS) WAF, Imperva, PerimeterX, and Vercel deployment protection, can return `403` or `429` to non-browser traffic even when the URL works in your browser. Doc Detective already sends a browser-like header set and retries transient failures, but some WAFs fingerprint deeper signals like TLS or source IP reputation that header spoofing can't influence.
