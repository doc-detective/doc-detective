...
- `headers`: (Optional) Additional HTTP headers to include in the request, merged on top of Doc Detective's default browser-mimicking headers. Accepts either a key/value object or a newline-separated string (for example, `X-Api-Key: abc123\nAuthorization: Bearer token`). Useful for passing bypass tokens (such as Vercel protection bypass) or other authentication headers required by sites that block automated requests. Defaults to `{}`.
...
Some websites block automated HTTP requests with bot protection or rate limiting. Doc Detective automatically retries failed requests up to three times with exponential backoff (1 second, then 2 seconds, then 4 seconds) and attempts a `HEAD` request as a fallback for `429` or `403` responses. If the request still fails after these automatic retries, `checkLink` reports diagnostic information to help identify the cause.
...
For sites with other protection mechanisms, you have several options:
...
- **Widen `statusCodes`** to accept the WAF response code, for example `"statusCodes": [200, 301, 302, 307, 308, 403, 429]`. This is pragmatic but loses signal, because the check only confirms that the URL exists.
- **Use browser-based actions** like `goTo` and `find` instead. These can handle JavaScript challenges and pass full browser fingerprint checks.
