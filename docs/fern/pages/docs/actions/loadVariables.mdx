...
<Warning>
A `$VAR` reference resolves to its value in your test results, logs, and reports. For a credential, reference it as `$secret.API_KEY` instead — Doc Detective reads the same variable but keeps the value out of everything it writes. Load it the same way. See [Secrets](/docs/test-docs/secrets).
</Warning>
