...
<Warning>
When using concurrent test runners (`concurrentRunners` > 1), simultaneous screen recordings can capture the wrong browser window. Keep `concurrentRunners` at `1` for tests that include recording.
</Warning>
