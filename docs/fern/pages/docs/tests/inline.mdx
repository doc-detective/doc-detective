...
Inline tests also make maintenance easier. You update them alongside the content they test, so the two stay in sync as the content changes.
...
- **Straightforward maintenance**: When you update documentation, the associated tests are immediately visible and can be updated at the same time.
