...
Doc Detective can automatically generate tests based on your documentation source files and your `fileTypes` configuration. Detected tests are useful for large, complex test suites that you want to keep in sync with your documentation. To enable test detection, set `detectSteps` to `true` and define markup patterns and associated actions in the `fileTypes.markup` array of your [config](/reference/schemas/config). Doc Detective uses those to extract steps from your doc source files. You can define multiple test patterns in your config to extract different types of tests from your documentation.
...
> You can mix detected tests with [inline tests](/docs/test-docs/inline) to declare steps your content might not cover, such starting or stopping a recording.
...
Combine detected tests with [inline tests](/docs/test-docs/inline) for comprehensive test coverage with minimal manual effort.

## Default markup patterns

Doc Detective includes default markup patterns for Markdown and DITA files. For comprehensive information about each input format and their patterns, see [Input formats](/docs/test-docs/input-formats/overview).
...
For the complete Markdown pattern configuration and detailed examples, see [Markdown input format](/docs/test-docs/input-formats/markdown).
...
For the complete DITA pattern configuration and semantic element reference, see [DITA input format](/docs/test-docs/input-formats/dita).
...
If you only need to perform basic steps, you can define actions using a shorthand of only specifying the action name. Doc Detective uses default actions. It substitutes the first capture group from the regex pattern into the most common location for the action. When there are no capture groups, it substitutes the entire match.
