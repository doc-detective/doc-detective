...
Your docs show an API request and the response a reader should expect. Doc Detective can issue that request and confirm the response still matches. This guide walks a complete journey. Make a request, assert on its status and body, capture a value from the response, and reuse that value in a follow-up request. It also covers validating links with `checkLink`.
...
- Set `response.required` to assert that fields exist without pinning their values. That suits timestamps, IDs, and tokens that change every run.
...
To chain calls, such as creating something and then fetching it, capture part of the response with the step-level `variables` object. Reference the variable with `$NAME` in a later step.
...
- [Generate requests from an OpenAPI description](/docs/test-docs/generate-tests-from-openapi) instead of hand-writing each one, and override the `Authorization` header for authenticated APIs.
