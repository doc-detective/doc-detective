...
A [test](/docs/get-started/how-testing-works) in Doc Detective is a series of steps, where each step performs a single [action](/docs/actions). An action can be navigating to a URL, finding an element, or taking a screenshot, for example.
...
5.  In your terminal, enter the following command to run the test:
    ...
    <Tabs>
      ...
      {/* Illustrative CLI invocation; not run in the docs gate — it runs a nested Doc Detective against the empty homepage-check.spec.json created above, which would fail and could trigger a browser run. */}
      {/* test ignore start */}
