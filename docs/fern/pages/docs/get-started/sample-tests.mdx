...
**Note**: By default, paths inside a config file resolve relative to the config file's location, not the current working directory. This is controlled by [`relativePathBase`](/reference/schemas/config), which defaults to `file`; set it to `cwd` to resolve config paths relative to the current working directory instead. Paths passed on the command line with `--input`, `--output`, or `--config` always resolve relative to the current working directory.
