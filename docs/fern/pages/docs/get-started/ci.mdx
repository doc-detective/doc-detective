...
| `DOC_DETECTIVE_AUTOINSTALL` | Set to `0` (or `false`, `no`, `off`) to skip pre-installing heavy runtime assets during `npm install`. Assets will install lazily on first use instead. |
