...
If tests fail, you can automatically create a GitHub issue to track the problem. You can also trigger [integrations](/docs/test-code/integrations) to investigate and fix failures.
...
| `integrations` | None | Comma-separated integrations to trigger (see [Integrations](/docs/test-code/integrations)) |
...
Setting `env` at the job level makes `DOC_DETECTIVE_CACHE_DIR` visible to every step, so both the pre-install step and the `doc-detective/github-action@v1` step use the same cached directory.
...
| `DOC_DETECTIVE_AUTOINSTALL` | Set to `0` (or `false`, `no`, `off`) to skip pre-installing heavy runtime assets during `npm install`. Assets install lazily on first use instead. |
