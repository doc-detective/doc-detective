...
concurrentRunners | integer,boolean | Optional. Number of concurrent test runners to execute contexts in parallel. Set to `true` to use your CPU core count (capped at 4), or specify an explicit integer. The default of `1` runs contexts sequentially. Equivalent to `--concurrent-runners` on the CLI.<br/><br/>**Limitations:** Cross-context variable dependencies (via `variables` steps) are not supported when runners > 1, as variables write to `process.env` shared across contexts. Tests using `record` steps should keep this at `1` because concurrent screen recordings can capture the wrong window.<br/><br/>Minimum: 1 | `1`
