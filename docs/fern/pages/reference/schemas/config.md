...
reporters | array of string | Optional. Reporters to use when emitting test results. Built-in reporters: `terminal`, `json`, `html`, `runFolder`. The `runFolder` reporter (enabled by default) archives each run's results as `<output>/.doc-detective/run-<runId>/testResults.json` with a matching `testResults.html` for human-readable viewing, beside any screenshots the run captured. This is in addition to the flat output the `json` reporter writes. The per-run `<output>/.doc-detective/run-<runId>/` folder is only created when a run writes to it—when the `runFolder` reporter is active or `autoScreenshot` is enabled—so a reporter set that excludes `runFolder` (with `autoScreenshot` off) leaves no empty run folders behind. Custom reporters registered via `registerReporter()` can also be referenced by name. | ``["terminal","json","runFolder"]``
