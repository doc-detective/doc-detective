...
- [docs/content-strategy/](docs/content-strategy/): audiences, personas, CUJs, and the information architecture that govern every documentation change. Consult it before writing docs (see ["Documentation content strategy"](#documentation-content-strategy-required)).
...
Do this *first*, before `npm ci`, before browser and driver installs, and before touching code. You
then build and test against the current tree rather than a stale snapshot. Resolve any conflicts, then proceed to
dependency install below.

**Install dependencies before you start working.** A fresh clone or git worktree has no
`node_modules`, since it's gitignored and not shared between worktrees. Tests, the local
...
notable one) while most are LF, and `core.autocrlf=input` re-normalizes on `git add`. Some scripts
read and rewrite a whole file, such as `python`'s `io.open(...).read()` and `write()`, or any
read-modify-write helper. Those silently convert CRLF → LF. That turns a one-line change into a
...
Instead, record durable knowledge **in the repo, in the same change**, choosing the home by kind.
That covers a gotcha, a triage rule, a recipe, a decision, or a preference the user states:
...
- **Prefer agent-native SKILL.md over heavy third-party installs.** When asked to integrate a
  third-party agentic tool, such as a Claude Code add-on or orchestration kit, research it first.
  Then offer two paths in the first response. Install upstream as written, or capture its intent as
  a `SKILL.md` driven by the agent's existing tool surface. Lean toward the SKILL.md when the
  upstream install is heavy (daemons, sudo, system binaries), OS-incompatible, or duplicates
...
1. **Red**: write a failing test that captures the desired behavior first. Run it to confirm it fails for the expected reason.
...
behavior, the reasoning, and the decision*. Write it **before** the code, or alongside it, so it is
the reviewable source of truth rather than an afterthought.

- **Format**: MADR 4.0.0. Include the YAML front matter (`status`, `date`, `decision-makers`) and
  the standard sections. Those are *Context and Problem Statement*, *Decision Drivers*, *Considered
  Options*, *Decision Outcome* (with *Consequences* and *Confirmation*), and *Pros and Cons of the
  Options*.
...
- **Scope**: ADRs document *decisions* such as behavior, contracts, and trade-offs, not mechanical
  changes. Pure refactors, dependency bumps, typo and doc fixes, and style changes don't need one.
  A change that alters observable behavior or a public contract does.

## Feature fixtures (required)

Unit tests are necessary but not sufficient. When you add or change a **user-facing feature**, also author **Doc Detective fixtures** that exercise it end-to-end through the real runner. Such a feature might be a new step type, action option, config or CLI flag, engine, or output format. Cover **every permutation** of it, not just the happy path.
...
- The graceful-degradation and guard paths (unsupported platform, headless, missing dependency).

Fixtures live in [test/core-artifacts/](test/core-artifacts) as `*.spec.json` files, organized into **per-feature subdirectories** (`navigation/`, `interactions/`, `capture/`, `recording/`, `routing/`, `guards/`, `http/`, `process/`, `sessions/`). In CI the subdirectories run grouped into **bundles**, one job per bundle and OS, through the reusable [.github/workflows/fixtures.yml](.github/workflows/fixtures.yml). Heavy or special-cased groups get a job to themselves. Fast groups share a job through a comma-joined `input`. See [ADR 01022](adrs/01022-parallel-feature-fixture-jobs.md) for the fan-out, and [ADR 01048](adrs/01048-reduce-pr-gate-latency.md) for the bundling that amends it. Each job builds the PR, `npm link`s it, and runs its bundle through the [Doc Detective GitHub Action](https://github.com/doc-detective/github-action) with `version: ''`. A bare `npx doc-detective` then resolves to the linked local build. It then fails on any FAILed or zero-spec run through [scripts/check-fixture-results.cjs](scripts/check-fixture-results.cjs). Every fixture must therefore resolve to **PASS** or **SKIPPED**, never FAIL. A deterministic **WARNING** is allowed only for features whose designed signal *is* a warning, such as recording staleness detection and screenshot variation. The gate fails a job only on FAILed specs or a fully empty run:

- **Put a new fixture in the subdirectory for its feature area.** For a genuinely new area, create a new `<group>/` directory **and add it to a bundle's `input` in [.github/workflows/fixtures.yml](.github/workflows/fixtures.yml)**. Join an existing bundle when the group is fast, keeping the bundle's worst leg, cold-cache Windows, well under its timeout. Add a new solo entry when it's heavy or needs its own gating env. Do the same when it's iOS-flavored, because the action's `ios: auto` scan can't see into comma-joined inputs. **Caution:** the zero-spec guard only fails a *fully* empty run. In a multi-directory bundle, a typo'd directory contributes zero specs while the others still produce results. Double-check `input` paths in review, since the fixture-output artifact shows which spec files actually ran.
- Gate display- and engine-specific permutations with `runOn`, covering platforms and headed or headless. Each then runs only where it can succeed, and lands as `SKIPPED` elsewhere. Recording fixtures are the worked example. ffmpeg permutations run headed on Windows, macOS, and Linux, while browser-engine permutations run only on headed Chrome (Windows and macOS). Headless Linux jobs skip the headed recording permutations. That's expected, and it passes the gate.
- Permutations that are *meant* to be skipped or guarded belong in fixtures too. That covers a headless skip, a name conflict, and `record: false`. Assert the SKIPPED behavior, don't omit it.
- When a fixture asserts on captured step outputs, read them in `runShell` through `process.env.MY_VAR`, using the step `variables` export to the environment. Do **not** use textual `$MY_VAR` substitution. Substitution splices raw values into the command string, where Windows paths' backslashes become JS escape sequences and quoting breaks. See [recording/recording-outputs.spec.json](test/core-artifacts/recording/recording-outputs.spec.json) for the pattern.
- Shared state is provisioned per job. Browser and http groups rely on the test servers the job starts (8092 and 8093 via [test/server/start.js](test/server/start.js)). Specs that need the `env` variables load them with `loadVariables: "../env"`, from the single `env` at the artifacts root. Group jobs use the lean [config.groups.json](test/core-artifacts/config.groups.json), which has no live external integrations.
- Some behavior needs a precise assertion the "no spec fails" gate can't express. One example is a preflight that must skip a test for a specific reason. Add a focused `it(...)` in [test/core-core.test.js](test/core-core.test.js). That file also runs the broad `test.spec.json` **smoke** under mocha, and keeps the programmatic control-flow assertions.

See [test/core-artifacts/recording/recording.spec.json](test/core-artifacts/recording/recording.spec.json), [recording/recording-permutations.spec.json](test/core-artifacts/recording/recording-permutations.spec.json), and [recording/autorecord.spec.json](test/core-artifacts/recording/autorecord.spec.json) for the canonical pattern. That means one spec per feature, one test per permutation, `runOn`-gated, and PASS or SKIPPED only.
...
> user can see, run, configure, or rely on? That means a step type, action option, config or CLI
> flag, engine, output format, default, supported platform, integration, or error and skip behavior.
...
- **If yes, it has documentation impact, and the docs work is part of this change's
  ...
  pages must change or be created. Use [docs/content-strategy/](docs/content-strategy/) and the
  ["Documentation content strategy"](#documentation-content-strategy-required) rule below. Land the
  docs change alongside the code. If docs live on a separate cadence, open a tracked issue and
...
As a rule of thumb, a change that warrants an **ADR** or a **feature fixture** almost always has
user-facing surface. It therefore almost always has docs impact. The three move together:
**behavior change → ADR + fixtures + docs assessment.**
...
not by document type**. Do not impose a Diátaxis tutorial, how-to, or reference split as the
organizing principle. Before drafting or restructuring **any** user-facing documentation, consult it:
...
1. **Identify the persona.** Wren (documentation engineer, lead), Diego (developer and API tester),
   Priya (CI and platform engineer), Aria (AI-assisted author), or Cole (contributor).
2. **Find the matching CUJ** in [cujs.md](docs/content-strategy/cujs.md). That's W1–W3, D1–D3, P1–P3,
   A1–A2, C1, or the cross-cutting X1. Sequence the content by that journey.
...
5. **Match the source of truth.** Reference pages must agree with the code. See the source-of-truth
   ...
   authoring conventions such as Fern frontmatter, MDX, and Vale or Google style, follow
...
Pick the commit type deliberately. It is the **only** signal that decides whether a release is cut, and how.
...
**Keep transient files inside the worktree, never in system temp directories.** Put scratch output
files, throwaway `--cache-dir` targets, and downloaded archives for inspection all under
`.tmp/` at the repo root, which is gitignored. They're then visible in the worktree, cleaned up with
it, and never orphaned in `%TEMP%` or `/tmp`.
...
and remove them in `after` or `afterEach`. Those are uniquely named, never read by a human, and
cleaned up by the test that made them. That's the suite-wide convention across 150+ call sites.
Follow it in new tests rather than reinventing a `.tmp/` variant for one file.

Running tests is time-intensive. Rather than running a test several times to check different behaviors, save output to a file and inspect that file. Checking `tail` for output verification is one such case:
...
Every user-facing knob flows through the merged `config` object. CLI flags do **not** bypass it. They override it. Runtime code, meaning the runner, reporters, and integrations, reads from `config` only, never from `args`. That's what lets config files and `DOC_DETECTIVE_CONFIG` reach the same code paths the CLI does.
...
This is the convention for **new** strict-array fields. `--input` predates it. For backward compatibility it uses a permissive `stringOrArray` shape, accepting a string or an array with no item-level constraints. Don't model new flags on `--input`.
...
- **Yargs**: `type: "string"`, a single comma-separated value. Not `type: "array"`. The comma split lives in `setConfig`, so file and env users land on the same array shape.
- **`setConfig` override**: split on `,`, trim each, **drop empties**, store as `string[]`. See the `--test` and `--spec` blocks for the canonical pattern. `--input` splits and trims but does not drop empties, because its laxer shape predates this convention.
- **Runtime helper**: defense-in-depth. Also trim and drop empty entries inside the helper, since env and CLI paths could in theory bypass AJV.
...
See [PR #286](https://github.com/doc-detective/doc-detective/pull/286), the `--test` and `--spec` filters, for a worked example covering all five.

### Don't

- ❌ Don't read `args.foo` from runner or reporter code. Read `config.foo`.
...
- [.releaserc.json](.releaserc.json): semantic-release config (branches and plugins)
...
- [.github/workflows/npm-test.yaml](.github/workflows/npm-test.yaml): matrix tests, plus post-release Docker and downstream jobs
