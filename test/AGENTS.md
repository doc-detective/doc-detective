...
| A `fixtures/proc-sessions (windows)` leg fails `sessions/start-surface-parallel` (`start-surface-parallel-mixed`) with `Element not found within timeout` on the `find` for `PAGE par1` against the parallel-opened `par-chrome` session | **Suspected product bug, not a flake — do not rerun-and-forget.** The preceding `startSurface` and the surface-less `goTo` both report PASS, yet `par-chrome` never shows the page, so the `goTo` appears to navigate a different session than the one the `find` is scoped to (the authored-order activation this test exists to assert). Windows-only: the same spec passes on ubuntu in the same run, and `start-surface-parallel-two-browsers` (two same-engine sessions) passes while the mixed browser+process array fails. **A longer timeout is not the answer and was tried:** raising that `find` to 30s made it consume the full 30s and still find nothing. Tracked in [#696](https://github.com/doc-detective/doc-detective/issues/696) | Pull the `fixture-output-proc-sessions-windows-latest` artifact and check the failing step's `durationMs`. If it equals the find's timeout, the element is genuinely absent from that surface — investigate activation/session routing in [../src/core/tests/startSurface.ts](../src/core/tests/startSurface.ts) (cross-kind re-assert) and the surface resolution in [../src/core/tests/findElement.ts](../src/core/tests/findElement.ts), rather than adjusting the fixture |
