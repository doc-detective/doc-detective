...
| A `fixtures / proc-sessions (windows-latest)` leg fails `sessions/start-surface-parallel` (`start-surface-parallel-mixed`) with `Element not found within timeout` on the `find` for `PAGE par1` against the parallel-opened `par-chrome` session | **Root-caused — [ADR 01088](../adrs/01088-goto-verifies-it-left-the-blank-document.md), tracked in [#696](https://github.com/doc-detective/doc-detective/issues/696).** A fresh Chromium session on `windows-latest` occasionally stays parked on `data:,`, its initial blank document ([ADR 01084](../adrs/01084-retry-unnavigated-context.md)): alive, not a crash page, simply never navigated. Every `goTo` wait condition passes trivially against that blank page, so the navigation reported success and the failure surfaced later on a `find` that had nothing to find. 01084's retry probes the context's PRIMARY session only, so `par-chrome` — a SECONDARY session from `startSurface` — was never covered. Two wrong diagnoses were burned first: a cold-session timeout (disproved — a 30s find consumed the full 30s) and a routing/activation bug (disproved by audit, then conclusively by the fixture's reordered firefox assertion PASSing) | `goTo` now verifies it left `data:,`, re-issues once, and FAILs with a clear message otherwise. **If this signature reappears, the failing step should now be the `goTo`, not a later `find`** — if it is still a `find`, that is a NEW mode: pull the `fixture-output-proc-sessions-windows-latest` artifact and read which step failed first. Never reach for a bigger timeout here |
