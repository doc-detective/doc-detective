...
| A `fixtures/proc-sessions (windows)` leg fails `sessions/start-surface-parallel` (`start-surface-parallel-mixed`) with `Element not found within timeout` (~5.2s) on the first `find` against the freshly parallel-opened `par-chrome` session, right after a cross-session `goTo` | Cold parallel Chrome session on a loaded Windows runner needs more than the 5s default find timeout for its first interaction after cross-session navigation. Environmental, not code — observed on unrelated PRs (incl. schema-only diffs). Mitigated by explicit generous `find` timeouts in the spec (30s on first-find-after-parallel-open, matching the apps fixtures' pattern) | If it recurs *with* the explicit timeouts in place, it's no longer this signature — read the log for what the session was actually doing (session creation vs. navigation vs. render) before rerunning |
