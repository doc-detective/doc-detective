...
`failure`. There is nothing to fix in the code, and the only remedy is a human
clicking "Re-run failed jobs". This was observed on the macOS pool in PR #616,
run 29219105593. Two jobs were clipped in one sweep, one having executed 18 steps
and the other zero. It cost a manual rerun mid-merge-train. As the PR gate's job
...
* Never mask a genuine test failure, so a real red must stay red.
* Bounded: rerun at most once, no loops, no runaway runner spend.
* Don't hide a *systemic* pool problem behind silent reruns.

## Considered Options

* **A. Signature-gated auto-rerun through `workflow_run`** (chosen): detect the
...
* **C. Larger or self-hosted runners**: fewer losses, but real cost and
...
* **`cancelled > 0`.** With `fail-fast: false` on every matrix (test.yml,
  ...
  concluded `failure` with zero failed or timed-out jobs, and at least one cancelled job, can
  *only* be lost infrastructure. **No per-step heuristic is used.** The first
  ...
  shows a lost runner can be reaped after many steps, 18 here, so that condition
  produced false negatives. The run-level guards above are sufficient and
  strictly more correct.

`rerun-failed-jobs` re-runs `cancelled` jobs. That was verified: attempt 2 of the run
above re-ran both and passed. On a rerun the workflow writes a
`$GITHUB_STEP_SUMMARY` note naming the clipped jobs. Repeated reruns on one
pool then surface as a visible pattern, rather than being silently absorbed.
...
* Cost/watch: a rerun costs a second execution of the clipped jobs. If a pool
  ...
  summary is the signal to escalate, by pinning the image, switching pool, or
  filing with GitHub, instead of leaning on the rerun. Recorded in
