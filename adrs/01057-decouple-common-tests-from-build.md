...
the package also ran its full `c8 mocha` suite. The root `build`
(`build:common && compile && copy:schemas`) is invoked by **every** CI job that
needs the compiled artifacts. That's all ~40 fixture jobs, both mocha shards, and the
...
* **Flakiness.** `src/common/test/integration.test.js` does real module-load and
  fs work under a 10s timeout. On loaded macOS runners it intermittently
  exceeded it, failing the **build** step of otherwise-unrelated jobs. Reruns were
  observed on PRs #619, #620, and #621. A flake in one small suite could red any of
  ~40 jobs.
* **Wasted wall-clock.** Those jobs need the built `dist/`, not a test run. The
  suite added ~1 minute to the build step of every macOS fixture job. That build
  step is already ~3 min, on the scarce macOS pool.
...
* **A. Decouple** (chosen): `build` builds only, and the one job that needs the
  coverage runs `test:coverage` explicitly.
* **B. Bump only the integration test's timeout** to 60s. That removes the flake, but
...
* `src/common`'s `build` is now `dereferenceSchemas && generate:types &&
  compile`, with no `test:coverage`. Every `npm run build` and `build:common`
  consumer builds artifacts without running the suite.
* Only one consumer relied on the side effect: the `Coverage ratchet
  (src/common)` job in [`test.yml`](../.github/workflows/test.yml). It now runs
...
* Defense-in-depth: `integration.test.js`'s suite timeout is raised 10s → 60s.
  The jobs that still run it, the ratchet and the mocha matrix, then don't trip on
  a loaded runner.
* A regression guard (`test/common-build-contract.test.js`) asserts the build
  builds artifacts without invoking the suite, with no `test:coverage`, `mocha`,
  or `c8`. It still runs `compile`, and keeps a separate `test:coverage` entry.
  The guard is red against the prior build script, and green against the
  decoupled one.

### Consequences

* Good: the src/common suite no longer runs in ~40 fixture and shard jobs. One
...
* Good: coverage and the ratchet are unchanged. The suite still runs in the
  ratchet job, now explicitly, and in the mocha matrix.
* Neutral: one extra `run:` step in the ratchet job, the explicit test run,
  replacing the implicit one. That's the same total work there.
...
* Bad: it leaves ~40 jobs running the suite during build, so the waste and the
  coupling remain.
