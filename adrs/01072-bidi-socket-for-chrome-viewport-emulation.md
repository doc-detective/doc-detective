...
# Viewport emulation through a BiDi socket: evaluated and rejected (crashes recording)

## Context and Problem Statement

[ADR 01071](01071-warn-when-a-browser-floors-a-requested-viewport.md) made a floored viewport
*observable*, through a warning and realized-size reporting. But it did not make sub-floor viewports
*achievable*. A 375px request still renders at ~500px. Doc Detective sizes the viewport by
resizing the OS window, which is subject to the browser and OS minimum window width. The floor applies headless too.
...
Option 2, gating the socket to viewport contexts, does **not** rescue it. Recording and viewport are a
...
viewport would still get the socket and crash. There is no clean gate. Enabling the BiDi socket at
all reintroduces exactly the cross-driver instability ADR 00132 documented. The "narrow, socket-only"
hypothesis is false, because the socket's mere presence destabilizes the recording path.

Shipped behavior therefore remains ADR 01071. Resize the window, read the viewport back, and warn
when the browser or OS floored the request, on every engine. Sub-floor viewport fidelity through window
sizing is not achievable on this stack. Mobile-device emulation is the recommended path for true
small widths.
...
* Neutral: revisiting is possible if a future WebdriverIO or driver release makes the BiDi socket
  coexist with recording. It's also possible if a non-BiDi CDP path becomes reachable through Appium;
  chromedriver's `/goog/cdp/execute` currently 404s through the Appium proxy.
