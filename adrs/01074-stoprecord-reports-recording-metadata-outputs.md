...
A `stopRecord` step currently returns only `{ status, description }`. The produced video's path, duration, resolution, frame rate, and format are invisible in the results JSON. So nothing downstream can reason about what was actually produced: reporters, integrations, assertions, or the planned recording visual-regression layers. Screenshots set rich `outputs` (`screenshotPath`, `variation`, …), and recordings set none. How should `stopRecord` report what it produced? And where should the metadata come from, given that ffprobe is not bundled?

## Decision Drivers

- All three engines (browser/MediaRecorder, ffmpeg capture, Appium device) must report the same output shape.
- The metadata must describe the **final artifact**, meaning post-crop, post-transcode, and post-gif-scale, rather than the raw capture.
...
Chosen option: **1, probe the final artifact with `ffmpeg -i` and parse stderr**. Options 1 and 4 both measure the file users actually get, while options 2 and 3 don't; see below. Option 1 wins between them because it needs no new dependency. Option 4 costs a second ~70 MB heavy dep and installer surface, for metadata the bundled ffmpeg already reports. Both work identically for all three engines, including the device engine's native-mp4 path that never transcodes.

`ffmpeg -i` with no output file exits non-zero by design, so the probe parses stderr regardless of exit code. It retains the **head** of that stderr, the first 4 KB. ffmpeg prints the `Duration`/`Stream` metadata block first. A tail would let trailing demux warnings evict exactly what we came for. That's the pattern the transcode step uses, where the interesting lines come last.
...
- Bad, because stderr parsing is a contract with ffmpeg's human-readable output. Regexes are kept deliberately loose, and unit-tested against canned stderr for mp4, webm, and gif.
...
- `stopRecording` integration tests in `test/app-recording.test.js` run the real probe against a real generated mp4, asserting the full outputs shape. They also run it against a non-video payload, asserting the step stays PASS with probe-derived fields omitted.
...
- The feature fixture `test/core-artifacts/recording/recording-outputs.spec.json` exercises the outputs end-to-end through the real runner on headed Windows and macOS. It captures `$$recordingPath`/`$$format`/`$$duration`/`$$width`/`$$height`/`$$fps` through step `variables`. It fails the spec if any is missing or non-positive, for both mp4 (fps token) and gif (tbr fallback).
...
- Bad, because it reports intent rather than reality. A truncated or corrupt output file would still "report" full metadata, defeating the point.
