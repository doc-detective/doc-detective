...
`doc-detective lsp --stdio` subcommand. That is the right entry point for the Claude plugin, whose
launcher shim invokes the subcommand. But it is awkward for **standard editor LSP clients** like VS
Code, Neovim, and Zed. They expect to configure a single `command`, a bare executable, not a
...
* Keep a single implementation, so both entry points run the same server.
...
stdout cleanliness is preserved. No module in the LSP chain writes to stdout, and the AJV
strict-mode warnings from schema load go to stderr. The JSON-RPC stream stays uncorrupted, and the
protocol tests would fail otherwise.
...
fragile bandaid. Binding the streams explicitly fixes the coupling at its root, and is
launch-agnostic.
