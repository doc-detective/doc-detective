...
Three dedicated ubuntu jobs prove the android bootstrap paths on every PR. There's the
emulator-runner **reuse** leg (~8 min). There's the **managed-boot** leg (~15 min, `install android`
plus two boot cycles plus the A6 two-device spec). And there's the **action-lazy** leg (~7 min,
cold SDK bootstrap through the GitHub Action). That is ~30 runner-minutes per PR, on scarce
Linux-KVM runners, verifying flows that a documentation-fixture or mocha-test-only change cannot
alter. ADR 01055's `select` job already analyzes the change set to build the fixture matrix. The
same analysis can gate these legs, under the same silent-skip-proof discipline.
...
* **B. Trigger-list predicate**: run the legs only when a curated list of android-ish paths
  changes. That inverts the failure direction, since a missing entry silently skips the gate.
  Rejected.
...
* `androidLegsRelevant(changedFiles)` in
  ...
  * prose (`docs/`, `adrs/`, any `*.md`).
  Everything else is relevant: product code, `scripts/`, workflows, package manifests, and shared
  fixture infra (`env`, `config.groups.json`). So are **the test servers** (`test/server/**`), which
  the mobile-web legs reach from inside the emulator through 10.0.2.2. Empty change sets are relevant.
* Reuse ADR 01055's self-contained `select` job. It emits an `androidLegs` output, computed by
  the same `--android-legs` run of the script over the PR's changed files. Each of the three
  KVM jobs carries `needs: select` plus `if: needs.select.outputs.androidLegs != 'false'`. Job-level
  `if:` **can** read the `needs` context, unlike `matrix`, so this gates cleanly. There's no input
...
* Unit tests (`test/select-fixture-bundles.test.js`) pin the predicate, including the test-server
  and shared-infra relevance cases. They also assert all three KVM jobs carry the gate.
...
* Good: the failure direction is safe. Any unrecognized file keeps the legs on, and the safe-list is
  small, explicit, and unit-pinned.
* Neutral: product-code PRs, the majority, still run all three legs. That's deliberate, since those
  changes can plausibly reach the runtime, installer, or driver behavior the legs guard.
...
* Bad: it's conservative, and src-touching PRs get no savings. That's accepted.
