...
`teardown` calls the handle's `kill()`. For a PTY-backed handle that is *already dead*, `kill()` can
**reject**. That's the common case here, since an early exit is precisely why readiness failed.
Nothing catches it, so a rejection:
...
3. **strands the registry entry**, because `processRegistry.delete(name)` never runs. The next
...
* Teardown is *cleanup*, not the operation under test, so it must never change the step's verdict.
...
* **B. `try { await teardown(bg) } finally { processRegistry.delete(name) }`.** It deregisters, but
  the rejection still propagates, so (1) and (2) above remain.
...
readiness failed. The child already exited or never initialized, so there is, in practice,
nothing left alive to reap. Making the step throw to chase that hypothetical is a strictly
worse trade.
...
* **Neutral:** a genuinely-live process whose `kill()` fails is no longer swept. That's unreachable
  in practice; see the trade-off above.
* **Neutral:** the happy path and the successful-`kill()` path are byte-identical.

## Confirmation

* Unit: `test/background-process.test.js` has "a teardown kill() rejection neither escapes, masks the
  readiness error, nor strands the registry entry". It drives a handle whose `kill()` rejects. It
  asserts the call **returns** `FAIL`, that the description still carries the readiness error (`exit code 1`)
...
* Neutral: a live process with a failing `kill()` goes unswept, which is unreachable on this path.
