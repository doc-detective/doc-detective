...
`driver` field — but the Firefox descriptor carried only `{ name, version, path }`, so
...
* [src/core/tests/geckoDriver.ts](../src/core/tests/geckoDriver.ts): a new
  `GECKODRIVER_EXECUTABLE_ARGS` constant, passed to every desktop-pool Appium server.

### The scope of `--allow-insecure`, in two dimensions
...
* Neutral: desktop-pool Appium servers now carry one insecure-feature opt-in. Scoped as above.
...
* Bad (narrow): on a machine with *both* a managed geckodriver and a system one, sessions now use
  ...
  That is the intended trade — an ambient rescue is exactly the ambiguity this removes — and the
...
* Red→green unit tests: `getDriverCapabilities` sets the capability from `app.driver` and omits it
  when absent ([test/context-resolution.test.js](../test/context-resolution.test.js)); the Firefox
...
* Good: pins the managed binary by absolute path — no dependence on PATH ordering, filename, or an
  ambient install.
...
* Bad: still implicit — resolution depends on PATH ordering against any system geckodriver, which is
