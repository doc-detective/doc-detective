...
Doc Detective shipped four reporters: `terminal`, `json`, `html`, and `runFolder`. None produced a
...
`json` reporter writes. There's no second traversal and no second source of truth, so the three
reporters can never disagree about what happened. Each file is one builder plus a short writer that resolves a
...
- **`\`, `|`, newlines and `<` are escaped in Markdown.** A pipe splits a table cell, and a newline
  ends a row. A backslash already in the text would consume the pipe's escape. A description
...
The fixed filenames are `junit.xml` and `doc-detective-summary.md`. `junit.xml` is a strong enough
convention to keep unqualified. The summary is namespaced because `output` defaults to `"."`, so a
bare `--reporters markdown` writes into the repo root and must not clobber a file the user owns.
Both overwrite on each run. A CI artifact declaration names one path, so collision-suffixing would
pin that path to the first run's results. This is a deliberate exception to
...
`<testsuites>`. GitLab builds its suite-level rollup from the per-suite attributes, rather
...
* Good, because a failed doc test now shows up in the merge request test widget and the CI job
  summary. It's no longer only in the job log.
...
* Bad, because the Markdown reporter lists at most 100 failures before emitting a
  ...
  step summary over 1 MiB outright, rather than truncating it. Bounding both the row count and the
...
mapping and counts, plus XML metacharacter and control-character handling. Every well-formedness
claim is checked by parsing with a real XML parser. It also covers pipe and newline escaping, empty
runs, null results, holes in `contexts`, the write-beside-a-file path, and the never-throws
contract.
[test/core-artifacts/reporters/reporters.spec.json](../test/core-artifacts/reporters/reporters.spec.json)
runs both reporters through a nested CLI run on Windows, macOS and Linux. That includes a failing
run that must produce a `<failure>` element.
...
* Bad, because it adds CLI surface and a file round-trip for data already in memory. It also cannot
...
* Bad, because it creates a second source of truth that can disagree with the `json` reporter.
  That's the precise failure this design avoids.
