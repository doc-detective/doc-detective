...
Chosen option: **A**. Config-level declaration is the ergonomic users expect, and merging it onto every `context.openApi` puts it exactly where `httpRequest` validation already looks. Treating an unmatched expected header as a FAIL aligns header assertions with the rest of the response-assertion contract. An expected header that's absent or wrong is a failed expectation.
