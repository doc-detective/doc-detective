...
`screenshot_v3`'s `path` pattern accepted **any** Windows absolute path (`[A-Za-z]:[\/\\].*\.(?:png|PNG)`, where `.*` is unrestricted) but limited every other path to `[A-Za-z0-9_.\/\\-]`. A space, parentheses, `~`, an apostrophe, or any non-ASCII character anywhere in a macOS/Linux path failed `step_v3` validation:
...
- The one real constraint must keep being enforced at the schema layer, the only contract config-file users see. That constraint: the capture is a PNG, and the runtime encodes PNG.
- The schema must not pretend to be a security boundary it isn't. Traversal was already accepted, and path containment for URL references is a runtime guard in `saveScreenshot`.

## Considered Options

1. **Gate the extension only.** Accept any path ending in `.png` or `.PNG`, keeping the URL and `$VAR` branches.
...
The three branches now say exactly what the field means. That's an `http(s)` URL to a PNG, optionally with a query string for signed URLs, or **any path to a PNG**, or a `$VAR` reference.
...
- Good, because macOS and Linux projects in ordinary directories can take screenshots at all, even with spaces, accents, and parentheses. autoScreenshot and checkpoints stop failing on paths the user never typed.
...
- Bad, because a typo'd path is caught slightly later, at write time with an OS error, rather than by the pattern. But the pattern never caught misdirected paths anyway, only mis-charactered ones. The extension check still catches the common `.jpg` or `.gif` mistake.

### Confirmation

- Schema tests live in [src/common/test/validate.test.js](../src/common/test/validate.test.js), under `screenshot path pattern`. Absolute POSIX paths with a space, parentheses, `~`, an apostrophe, and non-ASCII are accepted. Every previously-valid form still is: relative, POSIX plain, Windows absolute with spaces, URL, URL with query, `$VAR`, and `.PNG`. And `.jpg`, `.gif`, `shot.png.exe`, and extensionless targets are still rejected.
- Verified against the pattern read directly out of the schema, not a hand-copy, before and after. The Windows and POSIX asymmetry is gone, and both non-PNG negatives still reject.
...
- Bad, because it fixes absolute paths while leaving *relative* paths restricted; `docs (v2)/shot.png` would still fail. That's the same bug, smaller.
