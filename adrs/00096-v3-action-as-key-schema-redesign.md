...
Through v1 and v2, a Doc Detective step named its action in a field. v1 used a free `action` string, and v2 pinned it with `action: { const: "goTo" }`. Both shapes meant a validator could not tell, from the object's *keys* alone, which action a step performed. Authors also wrote a redundant discriminator. The v3 redesign asked whether the **action itself should be the object key**, as in `{ "goTo": "https://…" }`. A step's shape would then be self-describing, with one and only one action key per step. And should each action get its own strict `*_v3` sub-schema? This is the schema-side foundation that the runner later adopts (`00100`).
...
Chosen option: **A**. In `step_v3` the **action is the key**, and there is no `action` field. An `anyOf` constraint requires exactly one recognized action key per step. `id` becomes `stepId`; `outputs` and `variables` are expressed through `patternProperties`. Each action materializes its own `*_v3` schema. The contract was built up across several commits and a series of PRs:
...
* Bad: a hard break from v2's `action: const` shape, requiring an auto-migration (`00097`) and a runner adoption (`00100`).
* Neutral: string/object shorthands (e.g. `find` as a string) add `anyOf` branches to keep authoring terse.

### Confirmation

Shipped across common commits listed above under v3.0.0-dev and PRs #105/#106/#108. It's verified by the per-action example fixtures bundled with each `*_v3` schema, and the common validate suite.
