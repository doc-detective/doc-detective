...
another Node program was impossible, whether a build script, a CI pipeline, or a higher-level tool.
...
Chosen option: **A**. Exporting a function that resolves config in-process is the direct way to
make the tool embeddable, while reusing the existing resolution code. The contract evolved across
two commits:
