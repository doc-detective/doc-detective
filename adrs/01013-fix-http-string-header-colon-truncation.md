...
That's one `Name: value` header per line, e.g. `"Content-Type: application/json\nAuthorization: Bearer token"`.
...
segments. Any value that itself contains a colon is truncated at its first internal colon. A line
...
trimmed text before the colon. The value is the trimmed remainder of the line, so colons in the
value are kept intact:
...
A unit test in [test/httprequest-coverage.test.js](../test/httprequest-coverage.test.js) is named
"keeps colons in string-header values (splits on the first colon only)". It asserts that a string
block `"X-Callback: https://example.com/cb\nAuthorization: Bearer t"` yields the full URL
`https://example.com/cb` for `X-Callback`, not the truncated `https`. It also yields `Bearer t` for
...
is unchanged. The same `"Name: value\nName: value"` shape is accepted, and only the buggy truncation
of colon-bearing values is fixed. That reference example uses colon-free values, so it is not
misleading and needs no edit. This fix restores the documented convenience form's intended behavior; no
