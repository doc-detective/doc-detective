...
tests, and contexts are buried as path segments under a media-type root, `screenshots/` or
...
How should the per-run artifact layout be organized? Each level of the run hierarchy (run →
spec → test → context) should be an addressable resource, whose folder contains everything recorded
at or below it. That must preserve the run-over-run comparison guarantee that makes the reporter
useful.
...
* Locality: media recorded for a context should live *inside* that context's folder. It belongs
  beside any other artifacts for the same context, not in a parallel media-typed tree.
* Run-over-run comparison: the same step and context must land on the same path relative to the run
  folder in every run. The report's `autoScreenshot` value is stored relative to `runDir`, and this
...
inside the context folder, while keeping the change surgical. The only behavioral edits are the
three path builders (`getRunOutputDir`, `captureAutoScreenshot`, `buildAutoRecordStep`) and the
reporter's runId derivation and confinement. The report's relativization code is untouched, so the
run-over-run comparison guarantee holds. The relative path simply grows the
...
`.doc-detective/` to `.doc-detective/runs/`. A stale or hostile stamped path directly under
`.doc-detective/` is rejected, and the reporter falls back to a fresh in-tree `runs/<id>` folder.

To stay under `MAX_PATH`, the per-id path-segment cap drops from 64 to 32 at the two media call
sites. That cap is `capPathSegment`, tail-keep so the trailing content hash survives. Three 32-char
id segments reclaim ~96 characters, more than offsetting the four added fixed segments (`runs/`, `specs/`,
`tests/`, `contexts/`).
...
* Good: the report contract is unchanged. `runId`, `runDir`, and relative `autoScreenshot` paths
  keep their meaning, and only their string values change shape.
* Neutral/Cost: this is an observable layout change. Pre-existing `run-<id>/` folders from older
  ...
  External tooling that hard-coded the old `run-<id>/screenshots/…` path must update. It's released
  as a `feat`, a minor bump.
...
* Unit (`test/run-artifacts.test.js`): `getRunOutputDir` resolves under `.doc-detective/runs/`, with
  the bare-timestamp folder name and ordinal-suffix collisions. The reporter writes into and confines
  to the `runs/` root, and derives `runId` as the bare basename. `buildAutoRecordStep` produces the
  nested `specs/…/tests/…/contexts/<ctx>/recordings/<ctx>.mp4` path. An over-long id is capped to 32
  chars, by prepending a deterministic 8-hex hash of the full id and keeping the tail. Two distinct
  ids that share a 32-char tail then still produce distinct, stable path segments.
* Feature fixture (`test/core-artifacts/autoscreenshot.spec.json`, runOn-gated, PASS/SKIPPED only).
  It exercises autoScreenshot end-to-end across the precedence permutations. Those are spec-level
  true inherited, a test-level `false` that overrides spec-level true and captures nothing, a
  truthy-string coercion, and headless capture. Verified on Windows with real Chrome: all four tests PASS and screenshots
...
* Bad, because it is exactly the lack of nuance the change is meant to fix. Specs, tests, and
  contexts are not addressable resources, and a context's media is split across parallel
  media-typed trees.
