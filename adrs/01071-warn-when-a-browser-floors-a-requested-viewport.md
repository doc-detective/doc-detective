...
and calls `setWindowSize(window + delta)`. Two code paths do this. There's the context-level
...
say `375px`, is silently clamped up. The resize call succeeds, the page loads, a screenshot is
saved, and the step reports **PASS**. Success was defined as "the browser reached the page
...
This was reported externally by Diana Payton, in "The Browser Had a Floor I Didn't Know About". A
...
* Screenshots and measurements should be reconcilable against ground truth. That's the size the page
...
1. **Read back, warn on mismatch, and report the realized size** (chosen). After resizing, re-read
   `window.innerWidth/innerHeight`. Compare against the request through a pure helper,
...
3. **Emulate the viewport** (`driver.setViewport` over a BiDi socket, or CDP device metrics) so it
   renders at the requested size regardless of the window floor. That would be the *fidelity* fix.
...
4. **Do nothing, or document the floor.** Rejected: the whole point of the report is that a silent
   mismatch is invisible. A docs footnote doesn't reach the author looking at a green run.

## Decision Outcome

Chosen: **option 1**. A new pure helper `viewportMismatchWarning(requested, actual, tolerance)`
lives in `src/core/utils.ts`. It compares only the dimensions the caller actually requested, so a
width-only request is never warned about height. It treats an unreadable actual dimension as a
mismatch, and absorbs
...
* Good: a floored viewport is now visible. A `warning` line names the requested vs rendered size,
  and `startSurface` outputs carry ground truth. The step still PASSes, since the floor isn't a
  failure.
...
* Trade-off: the 16px tolerance can hide a genuine floor smaller than a scrollbar. That's accepted.
  The dominant failure mode is a large mobile-width clamp of 100px or more, and a zero tolerance
  would warn on benign scrollbar shifts.
...
* Integration tests use a driver stub that models the window→viewport relationship, plus an optional
  floor; see `test/start-surface-viewport.test.js`. It warns and reports the floored size, stays
  silent on exact realization, and emits no viewport output when none was requested.
...
* The floor applies **headless too**: a headless-Chrome 400px request rendered at 518px in the same
  ...
  A sub-floor viewport also can't be asserted "realized" in a portable fixture. The deterministic
  automated coverage is the unit and integration tests above. The realized happy path, an
