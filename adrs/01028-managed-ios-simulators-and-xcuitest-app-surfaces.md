...
skip, resolved and installed `appium-xcuitest-driver`, and gated on `xcode-select`/`simctl`.
...
repaired first. One was an `npm ci`-breaking `optionalDependencies` entry with a stale lockfile.
Others were stale generated schema outputs, and a dead `mobileContextSkipReason` branch propped up
by a green-washing test. A `core-core` test asserted iOS SKIPs on every host, failing the
macOS unit leg. Finally, `installIos` had untested branches. Those are corrected in a preceding
...
- Keep the cross-platform unit suite green and the coverage ratchet intact. The
  effectful simctl paths are macOS-only, and exercised by the `apps-ios` fixtures rather than the
  unit suite.
...
3. Implement a Doc-Detective-owned `simctl` simulator registry, mirroring the Android
   emulator layer. Resolve the newest iPhone, or a named or created device, then boot or reuse
   it. Attach XCUITest by `udid`, share one session per simulator with `activateApp`
...
- `runContext` threads a run-level `simulatorRegistry`, parallel to `deviceRegistry`.
  ...
  injected simctl effect bundle. The `serverDeps.acquireDevice` closure dispatches to
  `acquireSimulator` for iOS. That's the same `{ entry:{name,udid} } | { skip }` shape Android
...
- Good: the `apps-ios` fixture leg is promoted to require ≥1 PASS on macOS, through
  `DD_FIXTURES_REQUIRE_PASS`. An environment regression can't hide as all-SKIPPED, and
  Windows and Linux stay skip-tolerant.
...
- Unit tests cover the pure `iosSimulator` layer: parsers, selection, `planSimulator-
  Acquisition` reuse/boot/create/skip, `acquireSimulator` memoization plus failure drop,
  and teardown. They also cover the generalized `startAppSurface` iOS path: udid and bundleId
  capabilities, shared session plus `activateApp`, unsupported-field FAILs, and acquire-skip FAIL.
- The `apps-ios` fixtures exercise the real end-to-end path on macOS. They run through
  `.github/workflows/fixtures.yml` (require-pass) and the single-leg
  `fixtures-debug.yml` workflow dispatch from non-macOS development hosts. They cover
  ...
  budget. The effectful `simctl create` is also a thin wrapper over the
...
- Bad, because it is non-deterministic across Xcode versions, and gives no reuse or teardown
  ownership. It also diverges from the Android model, leaving two mental models for one concept.
...
- Good, because launch-ownership makes the run-end sweep safe (never kills a pre-existing
  booted simulator).
- Bad, because it is more code than option 2. That's mitigated by mirroring the proven Android
  layer one-to-one.
