...
its window, or close it. The plan of record defines an eight-phase roadmap; see
[docs/design/native-app-surfaces.md](../docs/design/native-app-surfaces.md), which expands
multi-surface Phase 5. This ADR ships **phase A1**: native
**Windows** apps as the third surface kind. The schema is shaped up front, so the later phases
...
* One addressing model: apps must be ordinary surfaces (`startSurface` opens, `surface` targets,
  `closeSurface` closes). There's no context-level `apps` field, and no `goTo`-launches-apps; the
  design rejected both.
...
* Driver weight: the Windows driver must be lazy-installed like every other Appium driver.
  It's never bundled and never a lockfile entry, and its absence must gate with a SKIP, not fail.
...
* **Schema**, all additive. `startSurface_v3` gains an app descriptor, with reserved mobile fields
  and a free-form `driverOptions` capability passthrough. `surface_v3` gains an `app` branch, using
  the windows-no-tabs selector grammar. App readiness (`{ delayMs, find }`) lands on `waitUntil_v3` and
...
* **Driver = ordinary lazy heavy dep.** `appium-novawindows-driver` is declared in
  ...
  `APPIUM_HOME/node_modules` only when its manifest cache doesn't exist. So the preflight deletes a
  manifest that predates the driver, and the server's next start rediscovers reality. This is the
...
* **Per-context app server.** App sessions run on their own Appium server, `APPIUM_HOME`-homed
  where the driver actually resolves, in the shim or the runtime cache. App and browser sessions
  then never fight over one home. The server starts lazily on the first `startSurface`, and dies in
  the context sweep.
* **Preflight gates, steps fail.** `runContext` preflights app-driver tests, checking that the
  platform is Windows and the driver is present or installable. Unmet contexts land as **SKIPPED**
  with an actionable reason, the same gating semantics as `requires` (ADR 01020). Reserved
  descriptor fields (`device`, `install`, `activity`, unsupported `env`) FAIL loudly, naming the
  roadmap.
* **Actions:** `find`, `click`, `type`, and `screenshot` targeting `{ app: … }` run on the app
  session's driver, through the UIA mapping column. That maps `elementText` → `@Name`,
  `elementId` → an AutomationId fast path, and `elementAria` → ControlType. `selector` accepts native XPath/`~accessibilityId`
...
* Good, because the charmap fixture passes end-to-end on Windows, through launch → click → type
  (escape hatch) → find → capture with typed content visible → close. It SKIPs on Linux and macOS,
  so the PASS/SKIPPED fixture invariant holds on every CI leg.
...
* Bad, because a driver lazy-installed into the runtime cache pulls a second Appium copy into the
  cache, when Appium itself is shim-resolved. That's roughly once, and disk-only. It's accepted:
  co-locating driver and server home is what makes discovery deterministic.
* Bad, because star-matched `@Name` locators can land on a non-focusable label when several
  elements share a name; that's the charmap copy-box case. The escape hatch covers it, and
  role-carrying semantic criteria are a documented improvement for a later phase.
...
* Hermetic unit tests: `test/app-surface.test.js`, 35 tests. They cover pure helpers, preflight with
  injected deps, session lifecycle with fake drivers and servers, locator building, and teardown.
* Schema: positive/negative cases in `src/common/test/validate.test.js`
  ("native app surfaces (phase A1)").
* End-to-end: `test/core-artifacts/app-surfaces.spec.json` in the combined core pass. It has the
...
* Good, because every action, the session registry, and screenshots ride the existing
  Appium/WebDriver client. That's one architecture across web and native.
...
* Bad, because it conflates the environment matrix with provisioning. The multi-surface design
  rejected it, and the plan of record superseded it. Multi-app and multi-surface tests are
  inexpressible.
...
* Bad, because input synthesis without an accessibility tree gives no semantic element model, no
  waits, and no portability to the mobile phases. That's the opposite of the design's element
  vocabulary.
