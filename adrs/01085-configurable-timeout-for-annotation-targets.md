...
*documenting* the gap. It adds a find-vs-annotate comparison table, plus a troubleshooting entry
teaching authors the guard-`find` workaround. That codifies the asymmetry into the contract rather than
...
Chosen option: **1, `timeout` on the annotation's element target**. It is the same field, in
the same shape, with the same name, unit, and default as `find`. That is what makes the asymmetry
...
* `find_v3`'s detailed object bundles *action* fields with finding fields: `moveTo`, `click`,
  `type`, and `surface`. `{"outline": {"selector": "#x", "click": true}}` would validate. An annotation
  that clicks is not a thing.
* Draft-07's `additionalProperties: false` does not compose through `allOf`. Both `find_v3`'s object
  and `annotation_v3`'s `target_element_shape` set it, so they cannot be intersected. That's the same
...
* Dereferenced `find_v3` is 452 KB (it pulls in `click_v3` and `type_v3`). `annotate_v3` is 230 KB
  ...
  repeatedly. And `FindElementDetailed` currently generates as `{[k: string]: unknown}`. That's the
...
A field-level `$ref` into a shared components block avoids all three. That's the pattern `click_v3`
uses for `button` and `duration`. But it was judged more surface than the drift it removes for a
single field. The finding fields have already drifted across four schemas; `click_v3` is missing
`timeout` too. Unifying them is its own change, not a rider on this one.
...
* Bad/limit: `renderLayer` re-resolves **every** stored annotation on each render. That includes the
...
* Bad/limit: `timeout: 0` is inconsistent, and the split is **per resolution path, not per
  ...
  clobbering. That changes `find`, and belongs in its own ADR. It isn't papered over here, because
...
* Schema: positive and negative cases live in
...
* Runtime: [test/annotations-geometry.test.js](../test/annotations-geometry.test.js) pins
  `criteriaFromTarget`'s pass-through. It drives `resolveAnnotationRects` end-to-end against a
  never-matching driver, asserting a 300 ms target returns well under the old hardcoded 5000 ms.
* The failure path is unchanged. [test/annotate-step.test.js](../test/annotate-step.test.js) asserts
  a never-appearing target still FAILs, bounded by the requested deadline, with the stored annotation
  set left untouched.
* End-to-end: [test/core-artifacts/recording/annotate-timeout.spec.json](../test/core-artifacts/recording/annotate-timeout.spec.json)
  annotates an element that appears only after the 5000 ms default would have lapsed. It covers the
...
* Good, because it is byte-for-byte the field `find` already has, with nothing new to learn.
