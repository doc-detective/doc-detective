...
Browser-engine recordings are isolated by two keys derived from the context id. One is the Chrome window title the launch flag `--auto-select-desktop-capture-source` matches, `RECORD_ME_<contextId>`. The other is the directory Chrome downloads the captured `.webm` into, `<tmp>/doc-detective/recordings/<contextId>/`. That isolation was designed for concurrency *within* one run, where context ids are unique.

Context ids are **not** unique across processes; `default` and `windows-chrome` are the common cases. So two doc-detective processes on one machine share both keys. This is a real scenario, not a hypothetical. A developer may run the CLI while the test suite runs, and parallel worktree sessions may run mocha at the same time. [test/AGENTS.md](../test/AGENTS.md) already documents that as a hazard.

Measured on Windows with two concurrent runs, each recording `out.mp4` in context `default`:

- Both produced **byte-identical videos**, same size and same SHA-256, and both reported PASS. One run silently transcoded the other's download. The recording claimed to document flow B while showing flow A.
- The same collision explains an observed `stopRecord` failure, "Recording download timed out". `startRecording` deletes a stale download at the shared path before capture, so a peer's start can delete an in-flight download. Chrome then renames a second download to `out (1).webm`, where nothing waits for it. A partially-written peer download also transcodes as garbage (`Read error at pos. 37 … End of file`).
...
- The key must be stable for a run's lifetime. The title is set at launch and matched later at `getDisplayMedia` time, and the download path is computed at start and awaited at stop.
...
Chosen option: **1, include the process id in both keys**. One `recordingProcessToken()` helper is used by `browserCaptureTitle` and `browserDownloadDir`. Both keys already flow through those two functions from all four production call sites. So a single change keeps the launch flag, the `document.title` assignment, and the download path in agreement.

The pid is unique among live processes by construction, which is exactly the scope the collision spans, and it's stable for the run. Pid reuse across time is irrelevant. Only *concurrent* processes can collide, and concurrent processes have distinct pids.
...
There's a known adjacent limitation, unchanged and pre-existing. A session launched without record options still gets the shared default flag value `RECORD_ME`, a prefix of every real capture title. Such sessions never call `getDisplayMedia`, so the flag is inert for them.
...
- Empirical before/after with two concurrent runs on one machine, same context id and same basename (`out.mp4`):
  - before: 1 distinct video content across the two runs, byte-identical, which is the collision;
...
- Bad, because pids repeat over time. That's harmless here, since only concurrent runs can collide.
