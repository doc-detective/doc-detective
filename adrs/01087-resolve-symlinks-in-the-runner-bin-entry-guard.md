...
`isEntry` is `false`, `main()` is never called, and the module merely defines and exports its
functions. **Node exits 0 having done nothing and printed nothing.**
...
egress, or an unreachable `PUBLIC_APP_URL`. None of which were true. Diagnosing it took an extended
multi-session investigation that also produced [ADR 01045](01045-surface-spec-fetch-failures-in-finalize.md).
Its extra error reporting could never have fired here, since the code that would report was never reached.
...
* The failure mode must not be silent. A guard that evaluates false costs an exit-0 no-op with zero
  diagnostics, which is strictly worse than a crash.
...
* **A. Resolve `process.argv[1]` with `realpathSync` and convert with `pathToFileURL` before
  comparing** (chosen).
* **B. Drop the guard, always call `main()`, and have tests import a separate module.** That would
  require splitting the file into a library module plus a thin bin wrapper. `bin/doc-detective.js`
  does that, as a one-line `import "../dist/cli.js"`, and therefore never had this bug.
* **C. Compare basenames, or check `process.argv[1]` ends with `doc-detective-runner`.** Cheap, but
  it matches on a filename rather than an identity. A differently-named symlink, or a same-named file
...
cases in the same change. The `try/catch` covers an unresolvable `argv[1]`, from a deleted file or a
permission error. It falls back to "not the entrypoint", the pre-existing behavior, so the guard can
never throw during module load.

Option B is arguably the sturdier structure, and matches `bin/doc-detective.js`. But it is a larger
refactor of a file that is otherwise working. It also does not by itself prevent a future guard
...
* **Good:** the regression is now covered by a test that spawns the file *through a symlink*. That's
  the exact production shape, closing the coverage gap that let this ship.
...
* Manually reproduced before the fix: `node bin/runner-entrypoint.js` logged a fatal and exited 1,
  while `node <symlink-to-it>` produced no output and exited 0. That matched the Fly machine logs
  exactly.
...
* Bad: it's still an idiom that is easy to get subtly wrong in a future copy-paste. The
  comment and the regression test mitigate that.
...
* Bad: it matches on name rather than identity. A same-named file in another location, or a symlink
