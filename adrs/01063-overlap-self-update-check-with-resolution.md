...
It calls `checkForUpdate(currentVersion)`, an npm-registry GET bounded at 3 s. If a newer
version exists, it calls `selfUpdate(...)`, which `npm install -g`s the new version and re-execs the
process. This ran **serially, awaited before any other startup work**. So a run paid up to ~3 s of
registry latency up front, doing nothing else. That was before it even began detecting and resolving
tests (`docs/design/run-performance.md`, item 2.3).
...
The whole point of a startup self-update is that you don't run the old version's tests. Not when a
new version is one command away.
...
* **C. Leave it serial** and accept the latency.
...
a single barrier. That barrier sits immediately after `detectAndResolveTests` resolves, and
**before** the dry-run JSON print, the JIT preflight, and `runSpecs`. On a newer version,
`selfUpdate` re-execs (`process.exit`) at that barrier. So the parent never produces a test, or
dry-run output, when an update is pending. When auto-update is gated off, `updateJoin` is
`undefined` and the barrier is a no-op.

Option **B** was rejected as **incorrect**. `selfUpdate`'s re-exec path is `runChild` in
`src/runtime/selfUpdate.ts`. It spawns the new version with inherited stdio, and **awaits the child
...
* The registry check *starts* earlier (concurrent with resolution) but its result is *joined* before
  execution.
* If `newer && latest`, `selfUpdate` runs at the join and re-execs before the first test. That's an
  identical observable outcome to the old serial code, minus the latency.
...
* One deliberate ordering shift: `getResolvedTestsFromEnv` (the DOC_DETECTIVE_API fetch) now runs in
  ...
  the parent may perform that fetch and then re-exec. That's harmless, since the child re-fetches.
  The "update before any test executes" guarantee is unaffected, because that fetch executes no tests.

### Consequences

* Good: up to ~3 s of registry latency is hidden behind resolution on every auto-updating run.
* Good: no new config or preference plumbing. The existing `autoUpdate`/`SKIP`/`CI` gate is unchanged.
...
* Red→green unit test in `test/cli-index-adapters-coverage.test.js`, named
  ...
  That proves the barrier precedes both dry-run output and execution. On the pre-change code the option
...
* Bad: it's incorrect. `selfUpdate` runs the child to completion, so parent and child double-execute
  the run, and race on exit.
