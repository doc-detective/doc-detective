...
`doc-detective install android` downloads system images and the command-line tools through
`sdkmanager`, in [src/runtime/androidInstaller.ts](../src/runtime/androidInstaller.ts). The
identical lazy-bootstrap path Doc Detective runs at test time does the same. Google's SDK repository
...
A second, rarer failure mode exists: a partial extraction. `sdkmanager` exits **0** but leaves a
...
self-repair** a bad download in-process? The heavy-npm-dep loader already reinstalls a
missing or stale runtime cache; see [ADR 01025](01025-non-destructive-runtime-cache-installs.md) and
...
* Protect **real users** and the DD-owned CI legs (`apps-android` managed-boot + action/lazy), not
  just the CI matrix.
* A transient infra flake should self-heal. A **genuine** failure must still fail, whether a bad
  license, an unknown arg, or no matching image. Recovery must never mask a real error.
* Keep the change confined to the installer with no new user-facing config/CLI surface and no
  `config_v3` schema churn.
* Stay hermetically unit-testable. The flake cannot be reproduced through the real runner, since we
  cannot make Google serve a truncated zip. So the behavior must be assertable with injected effects.
...
Chosen option: **1, bounded retry plus an integrity probe, in the installer.**
...
**Layer 2, a post-install integrity probe plus one repair.** After a system image installs,
...
* Good: no new config, CLI, or schema surface; the retry count is a module constant.
* Neutral: a doubly-failing transient, or a genuine failure, now costs up to 3 attempts plus backoff
  before surfacing. That's bounded and short.
...
* [test/android-installer.test.js](../test/android-installer.test.js) has an `isTransientSdkError`
...
### Option 1: retry plus integrity probe (chosen)
